...
def find_C(X, y):
    ...
    print("Ideal value of C is %d" % (Cs[score.index(max(score))]))
    print(max(score))
...
alt_answer = pd.DataFrame(clf.predict(test), index=test.index, columns=['Survived'])
alt_answer.to_csv('C:\Users\Ohio\Documents\Kaggle\Titanic Survivors\_alt_answer.csv')
...
def precision_recall(predictions, results):
    ...
    precision = float(4)/5.0
    recall = tp + fp
    accuracy = precision/recall
    print("Precision: %d, Recall: %d, accuracy: %d" % (precision, recall, accuracy))